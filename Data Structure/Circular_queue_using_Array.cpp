...
public:
    ...
    Circular_queue(const Circular_queue& other) : max_size(other.max_size), arr(new T[max_size]) {
        this->arr = new T[other.max_size]; // Allocate a new array
        this->f = other.f;
        this->b = other.b;
        max_size = other.max_size;
        for (int i = 0; i < max_size; i++) { // copying each element
            this->arr[i] = other.arr[i];
        }
    }

    void operator= (const Circular_queue& other) {
